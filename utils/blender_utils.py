...
def enable_gpu_rendering(rendering_engine: str = "OPTIX", gpus: str | int | list[int] | None = None) -> None:
    """
    ...
    unique_pci_ids = list({pci_id for _, pci_id in gpu_devices})

    # Validate GPU indices if specific GPUs are requested
    if isinstance(gpus, list) and any(idx >= len(unique_pci_ids) for idx in gpus):
        raise ValueError(
