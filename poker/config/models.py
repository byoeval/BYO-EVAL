...
class CardModel:
    ...
    rotation_euler: Optional[Tuple[float, float, float]] = None  # Optional rotation in radians (X, Y, Z)
...
class RiverModel:
    ...
    scale: Union[float, Tuple[float, float, float]] = 0.1 # Uniform scale for all cards
    ...
    random_seed: Optional[int] = None # Optional seed for reproducibility of random elements
...
class PlayerHandModel:
    ...
    card_names: List[str] = field(default_factory=lambda: DEFAULT_CARD_NAMES) # Default value
    scale: Union[float, Tuple[float, float, float]] = 0.1 # Uniform scale for all cards in hand
    ...
    random_seed: Optional[int] = None # Optional seed for random elements

    def __post_init__(self):
        """Validate configuration after initialization."""
        if len(self.card_names) != self.n_cards:
            raise ValueError(f"Length of 'card_names' ({len(self.card_names)}) must equal 'n_cards' ({self.n_cards})")
        if not (0 <= self.n_cards):
...
class CardDistributionInput:
    ...
    river_cards: Optional[int] = None
    # Can be a single int (apply to all players) or a list matching n_players
    player_cards: Optional[Union[int, List[int]]] = None
    # Allow specifying player cards via the variable map directly
    player_cards_map: Optional[Dict[str, int]] = None # e.g., {'players.0.hand_config.n_cards': 2}
    overall_cards: Optional[int] = None # NEW: Specify total cards directly
    min_cards_per_player: Optional[int] = None # NEW: Minimum cards each player should get
    n_verso: Optional[int] = 0 # NEW: Number of cards to be dealt face down (verso)
    max_player_cards: Optional[int] = 8 # NEW: Maximum cards any single player can receive
    max_river_cards: Optional[int] = 12 # NEW: Maximum cards the river can receive
    max_piles_per_player: Optional[int] = None # NEW: Maximum *piles* any single player can get (used with overall_piles)
...
class CardTypeModel:
    ...
    subset_n: Optional[int] = None # Required if mode is SUBSET_N
    card_list: Optional[List[str]] = None # Required if mode is EXPLICIT_LIST
    suit: Optional[str] = None # Required if mode is SUIT_ONLY (e.g., "Spades", "Hearts")
...
class ChipDistributionInput:
    ...
    n_players: Optional[int] = None # Provided by the context
    overall_piles: Optional[int] = None # Specify total *piles* for all players
    piles_per_player: Optional[Union[int, List[int]]] = None # Specify *piles* per player (int or list)
    min_piles_per_player: Optional[int] = None # Minimum *piles* each player should get
    max_piles_per_player: Optional[int] = None # NEW: Maximum *piles* any single player can get (used with overall_piles)
    # --- NEW: Options for randomizing pile properties ---
    n_chips_options: Optional[List[int]] = None # List of possible chip counts per pile
    color_options: Optional[List[Optional[Tuple[float, float, float, float]]]] = None # List of possible RGBA colors (or None for default)
    scale_options: Optional[List[Union[float, Tuple[float, float, float]]]] = None # List of possible scales (float or tuple)
...
class ChipModel:
    ...
    scale: Union[float, Tuple[float, float, float]] = 0.06 # Default scale
    color: Optional[Tuple[float, float, float, float]] = None # Optional RGBA color override
    blend_file_path: Optional[str] = None # Optional path to chip blend file
...
class ChipPileModel:
    ...
    n_chips: int
    base_chip_config: Union[Dict[str, Any], ChipModel] # Config for chip type (location ignored)
...
class ChipAreaConfig:
    ...
    base_pile_config: Union[Dict[str, Any], ChipPileModel] # Default pile settings (location ignored)
    n_piles: int = 1 # Number of piles()
    # --- Optional overrides per pile (lists must match n_piles if provided) --- 
    n_chips_per_pile: Optional[List[int]] = None 
    pile_colors: Optional[List[Optional[Tuple[float, float, float, float]]]] = None
    pile_spreads: Optional[List[Optional[float]]] = None
    ...
    random_seed: Optional[int] = None # Seed for chip pile layout randomization
...
class PlayerModel:
    ...
    player_id: Union[str, int] = "Player"
    hand_config: Union[Dict[str, Any], PlayerHandModel] = field(default_factory=dict)
    chip_area_config: Optional[Union[Dict[str, Any], ChipAreaConfig]] = None
    ...
    _resolved_pile_configs: Optional[List[Dict[str, Any]]] = field(init=False, repr=False, default=None)
        ...
        if isinstance(chip_area_conf_dict, ChipAreaConfig):
            ...
            # Assuming ChipAreaConfig will have a to_dict
             try:
                 ...
                 pass # Pass the original dict for now
...
class CardOverlapModel:
    ...
    n_lines: Optional[int] = None # Required if layout_mode is HORIZONTAL
    n_columns: Optional[int] = None # Required if layout_mode is VERTICAL
    ...
    # --- Misc --- 
    layout_id: Optional[str] = None # Optional identifier
    random_seed: Optional[int] = None # Seed for randomness

    def __post_init__(self):
        """Validate the configuration."""
        # --- DEBUG PRINT --- 
        print(f"[CardOverlapModel __post_init__] Validating instance:")
    ...
    def from_dict(cls, config: Dict[str, Any]) -> 'CardOverlapModel':
        ...
        # --- DEBUG PRINT --- 
        print(f"[CardOverlapModel from_dict] Created instance with:")
...
class PlayerDistribution:
    ...
    n_players: Optional[int] = None # Explicit number of players (can override PokerSceneModel.n_players for distribution logic)
    spacing_degrees: Optional[float] = None # Override automatic spacing with a fixed angle
    ...
    @classmethod
    def from_dict(cls, config: Optional[Dict[str, Any]]) -> 'PlayerDistribution':
...
class PokerSceneModel:
    ...
    players: List[Union[Dict[str, Any], PlayerModel]] = field(default_factory=list) 
    
    community_cards: Optional[Union[Dict[str, Any], RiverModel]] = None # Config for board cards
    scene_setup: Dict[str, Any] = field(default_factory=dict) # Config for general_setup
    noise_config: Optional[Dict[str, Any]] = None  # Optional noise effects configuration
    deck_blend_file: Optional[str] = None # Optional override for deck file
    random_seed: Optional[int] = None # Optional top-level random seed
    card_type_config: Optional[Dict[str, Any]] = None # NEW: Configuration for deck restrictions

    # --- Fields for automatic generation (used if 'players' is empty) ---
    default_player_config: Optional[Union[Dict[str, Any], PlayerModel]] = None
    player_distribution_config: Optional[Dict[str, Any]] = field(default_factory=lambda: {"layout": "circular"})
    card_distribution_inputs: Optional[Dict[str, Any]] = None # Placeholder
    chip_distribution_inputs: Optional[Dict[str, Any]] = None # NEW: Inputs for chip pile distribution
    ...
    _resolved_community_cards: Optional[RiverModel] = field(init=False, repr=False, default=None)

    # New field for the overlap builder
    card_overlap_config: Optional[CardOverlapModel] = None
